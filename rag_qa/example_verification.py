"""
自验证流水线使用示例
展示如何使用 SelfVerificationPipeline 提高答案质量
"""
import os
import sys
...
# 创建自验证流水线
pipeline = SelfVerificationPipeline(
    llm=llm,
...
result = pipeline.generate_verified_answer(
    question=question,
    context=context,
...
verification = pipeline.verify_answer(
    context=context,
    question=question,
...
print(f"答案: {answer}")
print("\n步骤 1: 提取声明")
claims = pipeline.extract_claims(answer)
for i, claim in enumerate(claims, 1):
    print(f"  声明 {i}: {claim}")

print("\n步骤 2: 验证每个声明")
verifications = pipeline.verify_claims(context2, claims)
